...
- *Linear Search* (tìm kiếm tuần tự): duyệt từng phần tử, so sánh tới khi tìm thấy hoặc hết mảng. `findStudentById` ở mục 1 chính là Linear Search — độ phức tạp $\mathcal{O}(n)$.
- *Binary Search* (tìm kiếm nhị phân): chỉ áp dụng được khi mảng *đã sắp xếp* theo khóa tìm kiếm. Cắt đôi phạm vi tìm kiếm mỗi lần so sánh — độ phức tạp $\mathcal{O}(\log n)$.
- Hiểu vì sao Binary Search nhanh hơn Linear Search nhưng đòi hỏi dữ liệu phải có thứ tự trước.
- Khái niệm cơ bản về độ phức tạp thuật toán: $\mathcal{O}(n)$ và $\mathcal{O}(\log n)$ khác nhau như thế nào khi $n$ lớn.
...
*Bài toán:* `findStudentById` hiện tại là Linear Search ($\mathcal{O}(n)$). Viết thêm 1 hàm tìm kiếm bằng *Binary Search* để so sánh.
